...
public class BeePathfinding extends Behavior<Bee> {
    ...
    protected boolean checkExtraStartConditions(ServerLevel level, Bee bee) {
        return (bee.getNavigation().isDone() && bee.getRandom().nextInt(10) == 0  || !bee.getBrain().getMemory(BrainierBeesRegistries.HIVE_POS.get()).isPresent());
    }

    @Override
    protected boolean canStillUse(ServerLevel serverLevel, Bee bee, long l) {
        return (bee.getNavigation().isInProgress()  ||  !bee.getBrain().getMemory(BrainierBeesRegistries.HIVE_POS.get()).isPresent());
    }
    ...
    protected void tick(ServerLevel serverLevel, Bee livingEntity, long l) {
        ...
        if ((livingEntity).hasNectar()) {
            livingEntity.getBrain().setMemory(BrainierBeesRegistries.POLLINATING_COOLDOWN.get(), 400);
        }
    }

    public static boolean blockCloserThan(Bee bee, BlockPos blockPos, int i) {
        return blockPos.closerThan(bee.getBrain().getMemory(BrainierBeesRegistries.HIVE_POS.get()).get().pos(), i);
    }
    ...
    public static void smartBeesTM(Bee beeEntity, CachedPathHolder cachedPathHolder) {
                ...
                beeEntity.blockPosition().distManhattan(cachedPathHolder.cachedPath.getTarget()) <= 4)
        {
            ...
            for(int attempt = 0; attempt < 11 || beeEntity.blockPosition().distManhattan(mutable) <= 5; attempt++) {
                ...
                if (beeEntity.getBrain().getMemory(BrainierBeesRegistries.HIVE_POS.get()).isEmpty()) {
                    ...
                    if (blockCloserThan(beeEntity, mutable, BrainierBeesConfig.MAX_WANDER_RADIUS) && world.getBlockState(new BlockPos(mutable.getX(), mutable.getY()-2, mutable.getZ())).isAir()) {
